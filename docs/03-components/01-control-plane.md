...
```yaml
# Secret is provided via K-ORC → PushSecret → OpenBao → ESO
apiVersion: v1
kind: Secret
metadata:
  name: k-orc-clouds-yaml
  namespace: orc-system
  # Owner Reference to ControlPlane CR for automatic cleanup
  ownerReferences:
    - apiVersion: c5c3.io/v1alpha1
      kind: ControlPlane
      name: production
  ...
  clouds.yaml: |
    clouds:
      openstack:
        ...
        # No passwords in secret - more secure than User/Password Auth
```
