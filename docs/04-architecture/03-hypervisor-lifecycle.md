...
The following diagram shows all states of a hypervisor node and the transitions between them. Each transition is triggered by a specific trigger and processed by a specific controller. For the agent and controller architecture, see [Component Interaction](./02-component-interaction.md).
...
| HA Event             | HA Agent detects hardware problem (see [High Availability](./04-high-availability.md))                                         | `ha`             |
...
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Node Failure            | HA Agent detects via LibVirt Events, automatically creates Eviction CRD (see [High Availability](./04-high-availability.md)) | No (node must be restored first) |
...
The eviction process is aborted if preflight checks fail. The Eviction CRD is updated with an appropriate condition (see [CRDs](./01-crds.md#eviction-crd-hypervisorc5c3iov1)).
...
3. Migration CRD is created and monitored (see [CRDs](./01-crds.md#migration-crd-hypervisorc5c3iov1alpha1))
