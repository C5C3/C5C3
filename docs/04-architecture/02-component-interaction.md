...
For CRD definitions referenced in this chapter, see [CRDs](./01-crds.md). For the multi-cluster architecture overview, see [Architecture Overview](../02-architecture-overview.md).
...
For CRD definitions (`Hypervisor`, `Eviction`, `Migration`, `OVSNode`), see [CRDs](./01-crds.md). For the HA Agent event handling, see [High Availability](./04-high-availability.md). For the OVS bridge layout, see [Network Architecture](./07-network.md#ovs-bridge-layout-per-hypervisor-node).
...
For details on storage connectivity (Ceph RBD), see [Storage Architecture](./06-storage.md). For hypervisor lifecycle states, see [Hypervisor Lifecycle](./03-hypervisor-lifecycle.md).
