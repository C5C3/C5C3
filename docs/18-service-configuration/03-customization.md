...
CobaltCore's CRD-driven configuration approach prioritizes type safety and validation. This page discusses the trade-offs between structured and raw configuration, documents current CRD coverage, and presents design options for configuration overrides.
...
| **Validation** | API server + operator validation | Only runtime validation (oslo.config) |
...
| **ML2/Network** | — | — | Type drivers, mechanism drivers, OVN connections | — | — | — |
| **Scheduling** | — | — | — | — | — | — |
...
The following options are design concepts for extending configuration beyond CRD fields. They are documented here for architectural completeness.
...
A combination is likely optimal:
...
| Fix a bug in OpenStack code | Not possible via config | Image patch (see [Patching](../17-container-images/03-patching.md)) |
...
This maps directly to `[ml2]` and `[ovn]` sections in `ml2_conf.ini`. The operator validates that mechanism drivers match the configured type drivers (e.g., `ovn` requires `geneve`).
