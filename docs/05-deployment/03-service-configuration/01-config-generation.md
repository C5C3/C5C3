...
For validation checks that occur during reconciliation (between Steps 2-5), see [Validation](./02-validation.md). For brownfield integration details, see [Brownfield Integration](../../06-operations/03-brownfield-integration.md).
...
**Important:** Credentials appear in the rendered ConfigMap (as connection strings) but **never** in the CRD spec. The CRD only contains `secretRef` pointers. For the full secret management architecture, see [Secret Management](../02-secret-management.md).
...
For the full upgrade lifecycle, see [Upgrades](../../06-operations/01-upgrades.md).
...
| libvirt | connection URI, hypervisor type | Hypervisor CRD (`domainCapabilities.hypervisorType`), see [Hypervisor Lifecycle](../../04-architecture/03-hypervisor-lifecycle.md) |
