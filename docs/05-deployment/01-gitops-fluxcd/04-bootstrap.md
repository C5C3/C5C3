...
| **Rollback**               | `git revert` for quick recovery (details see [Upgrade & Lifecycle](../../06-operations/01-upgrades.md))       |
...
| **Progressive Delivery**   | SemVer ranges for controlled upgrades (details see [Upgrade & Lifecycle](../../06-operations/01-upgrades.md)) |
...
See [Secret Management](../02-secret-management.md) for OpenBao initialization and bootstrap secret configuration. After the FluxCD bootstrap completes, the [Credential Lifecycle](./01-credential-lifecycle.md) takes over with Keystone bootstrapping, service user creation, and cross-cluster secret synchronization.
