...
Implements the config generation pipeline documented in [Config Generation](../18-service-configuration/01-config-generation.md). This package renders INI configuration files from Go structs — no template language is used.
...
The config package directly implements the pipeline from [Config Generation](../18-service-configuration/01-config-generation.md): CRD spec → resolve secrets → apply defaults → render INI → immutable ConfigMap.
Override mechanisms described in [Customization](../18-service-configuration/03-customization.md) (configOverrides, conf.d pattern) are supported via `MergeDefaults` with user-provided overrides taking precedence.
...
Provides helpers for ESO-based secret workflows. Operators never interact with OpenBao directly — they work exclusively with Kubernetes Secrets that ESO creates from OpenBao (see [Secret Management](../13-secret-management.md)).
...
**PushSecret pattern:** Some secrets are generated by operators at runtime (e.g., Fernet keys). These are written to a Kubernetes Secret and then pushed to OpenBao via a PushSecret CR for backup and cross-cluster distribution. See [Credential Lifecycle](../11-gitops-fluxcd/01-credential-lifecycle.md) for the full ESO/PushSecret flow.
...
During the container image build (see [Build Pipeline](../17-container-images/01-build-pipeline.md)), the `uv pip install` step is extended to include extra packages:
