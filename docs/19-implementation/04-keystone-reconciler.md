...
Verifies that ESO-provided Kubernetes Secrets exist before proceeding. These Secrets are created by ESO from OpenBao paths (see [Secret Management](../13-secret-management.md)):
...
For database migration patterns during upgrades, see [Upgrades](../14-upgrades.md).
...
3. **OpenBao backup** (optional) — Creates a PushSecret CR to back up Fernet keys to `kv-v2/openstack/keystone/fernet-keys` in OpenBao. See [Credential Lifecycle](../11-gitops-fluxcd/01-credential-lifecycle.md) for the PushSecret pattern.

### reconcileConfig()

Implements the config generation pipeline from [Config Generation](../18-service-configuration/01-config-generation.md):
...
If the config content changes, a new ConfigMap is created and the Deployment is updated to reference it, triggering a rolling restart. See [Validation](../18-service-configuration/02-validation.md) for how validation operates across the pipeline.
